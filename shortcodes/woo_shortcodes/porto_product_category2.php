...
add_shortcode( 'porto_product_category2', 'porto_shortcode_product_category2' );
add_action( 'vc_after_init', 'porto_load_product_category_shortcode2' );

function porto_shortcode_product_category2( $atts, $content = null ) {
	ob_start();
	$atts['shortcode'] = 'product_category';
	if (!empty($_GET['count'])) {
		$atts['count'] = $_GET['count'];
	}
	include 'porto_products.php';

	return ob_get_clean();
}

function porto_load_product_category_shortcode2() {
	...
	vc_map(
		array(
			'name'        => 'Porto ' . __( 'Product category2', 'js_composer' ),
			'base'        => 'porto_product_category2',
	...
	if ( ! class_exists( 'WPBakeryShortCode_Porto_Product_Category2' ) ) {
		class WPBakeryShortCode_Porto_Product_Category2 extends WPBakeryShortCode {
		}
	}
}
